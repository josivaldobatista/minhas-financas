...
public class UsuarioResourceTest {
	...
	public void deveAutenticarUmUsuario() throws Exception {
		...
		Usuario obj = Usuario.builder().id(1l).email(email).senha(senha).build();

		Mockito.when(service.autenticar(email, senha)).thenReturn(obj);
		String json = new ObjectMapper().writeValueAsString(dto);

		// Execução e Verificação
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post(BASE_API.concat("/autenticar"))
				.accept(JSON_APP).contentType(JSON_APP).content(json);
	...
	public void deveCriarUmUsuario() throws Exception {
		...
		Mockito.when(service.salvarUsuario(Mockito.any(Usuario.class))).thenReturn(obj);
		String json = new ObjectMapper().writeValueAsString(dto);

		// Execução e Verificação
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post(BASE_API)
				.accept(JSON_APP).contentType(JSON_APP).content(json);
	...
	@Test
	public void deveRetornaUmBadRequestAoTentarCriarUmUsuarioInvalido() throws Exception {
		// Cenário
		String email = "usuario@email.com";
		String senha = "123";
